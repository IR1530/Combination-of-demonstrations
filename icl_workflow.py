...
def load_model_tokenizer(model_name, single_precision):
    if model_name == 'meta-llama/llama-2-7b-hf':
        if not single_precision:
            model = LlamaForCausalLM.from_pretrained(model_name, cache_dir="/scratch/manish/hf_cache/")
        else:
            model = LlamaForCausalLM.from_pretrained(model_name, torch_dtype=torch.float16, cache_dir="/scratch/manish/hf_cache/")
