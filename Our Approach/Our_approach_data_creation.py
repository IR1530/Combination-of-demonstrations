...
results_df['queryk1k2'] = results_df['label'] == results_df['prediction']
results_df['queryk1k2'] = results_df['queryk1k2'].astype(bool)

results_df["k1"] = results_df.selected_examples.apply(lambda x:x[0])
results_df["k2"] = results_df.selected_examples.apply(lambda x:x[1])

results_df['queryk1'] = results_df['query'] + "<SEP>" + results_df['k1']
results_df['queryk2'] = results_df['query'] + "<SEP>" + results_df['k2']

with open('single_shot_mapping.json') as f:
    single_shot_mapping = json.load(f)

results_df['queryk1'] = results_df['queryk1'].map(single_shot_mapping).apply(lambda x:x['correct']).astype(bool)
results_df['queryk2'] = results_df['queryk2'].map(single_shot_mapping).apply(lambda x:x['correct']).astype(bool)

def is_compatible(row):
    return row['queryk1k2'] == 1 and row['queryk1'] == 1 and row['queryk2'] == 1

results_df['compatible'] = results_df.apply(is_compatible, axis=1)


compatible_count = results_df['compatible'].sum()
incompatible_count = len(results_df) - compatible_count

print(f"Number of compatible pairs: {compatible_count}")
print(f"Number of incompatible pairs: {incompatible_count}")

def generate_triplets(compatible_row, incompatible_df):
    query = compatible_row['query']
    positive = f"{compatible_row['k1']}<SEP>{compatible_row['k2']}"
    
    triplets = []
    for _, incompatible_row in incompatible_df.iterrows():
        negative = f"{incompatible_row['k1']}<SEP>{incompatible_row['k2']}"
...
compatible_df = results_df[results_df['compatible']]
incompatible_df = results_df[~results_df['compatible']]

for _, compatible_row in compatible_df.iterrows():
    triplets = generate_triplets(compatible_row, incompatible_df)
    all_triplets.extend(triplets)

# Create the triplets dataframe
triplets_df = pd.DataFrame(all_triplets, columns=['query', 'positive', 'negative'])

print(f"\nTotal number of triplets generated: {len(triplets_df)}")

triplets_df.to_csv("paired_triplets.csv", index = False)
