    ...
    for test_example in test_examples:
        ...
        for i in range(len(top_k_examples)):
            for j in range(i+1, len(top_k_examples)):
                one_shot_example = top_k_examples[i]
                two_shot_example = top_k_examples[j]

                prompt = f'{prompt_prefix}Review: {one_shot_example.text_a}\nSentiment: {classes[one_shot_example.label]}\n\n'
                prompt += f'Review: {two_shot_example.text_a}\nSentiment: {classes[two_shot_example.label]}\n\n'
...
def main():
    # Configurations
    dataset = 'sst2'
    K_max = 2  # For one-shot prediction
