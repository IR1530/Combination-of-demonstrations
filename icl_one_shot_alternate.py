...
def get_one_shot_example_kdtree(test_example, sbert_model, tree, dataset_dict):
    test_embedding = sbert_model.encode([test_example.text_a])
    _, top_k_indices = tree.query(test_embedding, k=1)
    one_shot_example = dataset_dict['train'][top_k_indices[0][0]]
...
def pred_batch(splt, prompt_prefix, prompt_suffix, tokenizer, indexes, dataset_dict, sbert_model, tree, max_rem_len, prompts, batch_size, model, class_idx, classes, device):
    ...
    for test_example in test_examples:
        one_shot_example = get_one_shot_example_kdtree(test_example, sbert_model, tree, dataset_dict)
        
        prompt = f'{prompt_prefix}Review: {test_example.text_a}\nReview: {one_shot_example.text_a}\nSentiment: '
